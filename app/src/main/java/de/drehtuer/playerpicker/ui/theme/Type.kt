...
import de.drehtuer.playerpicker.R
 ...
 * Material's slots - the design has its own vocabulary (a kicker, a card title,
 * a micro label) that does not map onto `titleLarge` and friends.
 */
@Immutable
data class PPTypography(
    /** Uppercase eyebrow over the home headline, in accent. */
...
fun ppTypography(): PPTypography = PPTypography(
    kicker = base.copy(
        fontSize = 12.sp, fontWeight = FontWeight.Bold, letterSpacing = 0.2.em,
    ),
    display = base.copy(
