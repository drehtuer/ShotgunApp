...
/**
 * The Shotgun! palette, transcribed verbatim from the `--pp-*` custom
 * properties in the design export (`design/Shotgun.dc.html`).
 *
 * The `PP` prefix is kept deliberately: it mirrors the `--pp-*` token names the
 * design still uses, so a colour can be traced between the two by name.
 *
