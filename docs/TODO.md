...
- [ ] Cut a first release once there is something to release - `v*` tag - which
      is also the first real exercise of the release and docs workflows. Both
      have been rebuilt but **neither has ever run**: the release path is
      checked by reading, the docs path by building the site locally with the
      same container image CI uses.
